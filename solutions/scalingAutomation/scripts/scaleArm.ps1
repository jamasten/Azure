...
try {
	...
	function TryForceLogOffUser {
		...
		Process {
			...
			try {
				Write-Log -HostPoolName $HostPoolName -Message "Force log off user: '$($Session.ActiveDirectoryUserName)', session ID: $SessionID"
				...
				Write-Log -HostPoolName $HostPoolName -Warn -Message "Failed to force log off user: '$($Session.ActiveDirectoryUserName)', session ID: $SessionID $($PSItem | Format-List -Force | Out-String)"
			}
		}
		End { }
	}
	...
	foreach ($VM in ($VMs.Values | Where-Object { $_.Instance.PowerState -ieq 'VM running' -and $_.SessionHost.AllowNewSession } | Sort-Object { $_.SessionHost.Session })) {
		...
		if ($SessionHost.Session) {
			...
			foreach ($Session in $VM.UserSessions) {
				...
				try {
					Write-Log -HostPoolName $HostPoolName -Message "Send a log off message to user: '$($Session.ActiveDirectoryUserName)', session ID: $SessionID"
					...
					Write-Log -HostPoolName $HostPoolName -Warn -Message "Failed to send a log off message to user: '$($Session.ActiveDirectoryUserName)', session ID: $SessionID $($PSItem | Format-List -Force | Out-String)"
				}
			}
